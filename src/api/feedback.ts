...
export const feedbackApi = {
  /**
   * 표정 피드백 조회 (동기 처리)
   * GET /api/feedback/{session_id}/expression-feedback
   *
  ...
  getExpressionFeedback: async (
    ...
    return apiRequest<ExpressionFeedbackResponse>(
      `/api/feedback/${sessionId}/expression-feedback`,
      { method: "GET" }
    );
  },
  ...
  getPoseFeedback: async (
    ...
    return apiRequest<PostureFeedbackResponse>(
      `/api/feedback/${sessionId}/pose-feedback`,
      { method: "GET" }
    );
  },

  /**
   * 목소리 피드백 조회 (준비 - BE 미구현)
   * GET /api/feedback/{session_id}/voice-feedback
   *
   * 스키마는 준비되어 있으나 BE에서 아직 구현되지 않았습니다.
   * BE 구현 완료 시 바로 사용 가능합니다.
   */
  getVoiceFeedback: async (
    sessionId: number
  ): Promise<VoiceFeedbackResponse> => {
    return apiRequest<VoiceFeedbackResponse>(
      `/api/feedback/${sessionId}/voice-feedback`,
      { method: "GET" }
    );
  },
